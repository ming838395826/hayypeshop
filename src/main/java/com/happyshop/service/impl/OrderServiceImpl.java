...
import com.happyshop.vo.OrderProductVo;
...
public class OrderServiceImpl implements IOrderService {
    ...
    private static AlipayTradeService tradeService;//支付宝服务
    static {
    ...
    public ServerResponse pay(Long orderNo,Integer userId,String path){
        ...
        String subject = new StringBuilder().append("快乐购扫码支付,订单号:").append(outTradeNo).toString();
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                //细节细节细节
                String qrPath = String.format(path+"/qr-%s.png",response.getOutTradeNo());//文件路径
                String qrFileName = String.format("qr-%s.png",response.getOutTradeNo());//文件名
                ZxingUtils.getQRCodeImge(response.getQrCode(), 256, qrPath);//支付宝封装好的 生成二维码

                File targetFile = new File(path,qrFileName);//上传二维码到图片服务器
                try {
    ...
    public ServerResponse aliCallback(Map<String,String> params){
        ...
        if(order == null){
            return ServerResponse.createByErrorMessage("非快乐购商城的订单,回调忽略");
        }
        if(order.getStatus() >= Const.OrderStatusEnum.PAID.getCode()){
